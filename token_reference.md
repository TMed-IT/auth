...
Mahora Auth のJWTは `__Host-auth_token` Cookieに保存され、認証ホスト以外には送信されません。各サブドメインへJWTや署名鍵を配布せず、ブラウザから認証サービスのセッションAPIを呼び出してください。

### 1. ログイン

1. `POST /auth/signin/google` を呼び出し、返された `authUrl` へ遷移します。
2. Google OAuth完了後、認証サービスがJWTを `__Host-auth_token` Cookieへ設定します。
3. ログイン後は `AUTH_TRUSTED_ORIGINS` に登録されたサブドメイン、またはサブドメインなしのトップドメインへ戻ります。

リクエストBodyの `redirect` を省略すると、サーバーが `Referer` または `Origin` から許可済みの戻り先を自動判別します。判別したURLはOAuth一時Cookieへ保存され、既存ユーザーはコールバック完了後、初回ユーザーは同意完了後に同じサブドメインへ戻ります。どの候補も許可リストと完全一致しない場合は、`AUTH_URL` から算出したサブドメインなしのトップドメインへフォールバックします。
...
利用するフロントエンドのサブドメインラベルを `AUTH_TRUSTED_ORIGINS` に登録し、次のように呼び出します。サブドメインなしのトップドメインは自動的に許可されます。
...
- JWTや `JWT_SECRET` を他サービスへ配布しないでください。
...
- 認証・CSRF CookieやJWTをログへ出力しないでください。
